      ...
      <div className="grid grid-cols-7 grid-rows-6 flex-grow">
        {daysInGrid.map((day, index) => {
          ...
          return (
            <div
              key={index}
              className={`border-e border-b border-slate-200 p-2 flex flex-col relative transition-all duration-200 ${
                isWeekend 
                  ? 'bg-slate-100/70' 
                  : `cursor-pointer group ${isCurrentMonth ? `${bgClass} hover:bg-blue-100 hover:shadow-inner` : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`
              }`}
            ...
            >
              ...
              <span className={`self-end font-bold text-sm mb-1 transition-all duration-200 
                              ${isToday ? 'bg-blue-600 text-white rounded-full h-7 w-7 flex items-center justify-center shadow-lg animate-pulse' : ''}`}>
                {day.getDate()}
              </span>
              {appCount > 0 && isCurrentMonth && !isWeekend && (
                <div className="mt-auto bg-blue-200 text-blue-800 text-center rounded text-xs p-1 font-semibold group-hover:bg-white transition-colors">
